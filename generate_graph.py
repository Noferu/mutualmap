...
top_user_cluster_id = max(cluster_degrees.items(), key=lambda x: x[1])[0]
top_user_cluster_degree = cluster_degrees[top_user_cluster_id]
top_user_cluster_centrality = top_user_cluster_degree / (largest_cluster_size - 1) * 100 if largest_cluster_size > 1 else 0
top_user_cluster_name = data[top_user_cluster_id].get("global_name") or data[top_user_cluster_id]["name"]

# ----------- Export HTML -----------
output_file = "discord_friends_network.html"
net.show(output_file)

# ----------- Inject stats using external HTML template -----------
with open("stats_box.html", "r", encoding="utf-8") as f:
    template = f.read()

central_html = "".join(
    f"<li>{data[uid].get('global_name') or data[uid]['name']} "
    f"<span style='color:gray;'>({degree} connexions, {score * 100:.1f}%)</span></li>"
